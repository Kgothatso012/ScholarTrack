...
export default function AttendanceReportsScreen({ navigation }: Props) {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'present': return DT.green2;
      case 'absent': return DT.red;
      case 'excused': return DT.amber;
      case 'late': return DT.blue;
  ...
  const s = StyleSheet.create({
    ...
    dateInput: { fontFamily: 'Syne_700Bold', fontSize: 14, color: DT.white, flex: 1 },
    filterBtn: { paddingHorizontal: 14, paddingVertical: 8, borderRadius: 10, backgroundColor: DT.blue },
    filterBtnText: { fontFamily: 'Syne_700Bold', fontSize: 11, fontWeight: '600', color: DT.white },
    sectionTitle: { fontFamily: 'Syne_700Bold', fontSize: 13, fontWeight: '700', color: DT.white, paddingHorizontal: 16, marginTop: 16, marginBottom: 12, letterSpacing: 0.5 },
    ...
    statusText: { fontFamily: 'Syne_700Bold', fontSize: 11, fontWeight: '600', color: DT.white, textTransform: 'capitalize' },
    actionBtns: { flexDirection: 'row', gap: 8 },
    actionBtn: { width: 36, height: 36, borderRadius: 18, justifyContent: 'center', alignItems: 'center' },
    summaryCard: { ...glass, marginHorizontal: 16, marginTop: 16, padding: 20 },
    summaryTitle: { fontFamily: 'Syne_700Bold', fontSize: 13, fontWeight: '700', color: DT.white, marginBottom: 14, letterSpacing: 0.5 },
    summaryRow: { flexDirection: 'row', justifyContent: 'space-around' },
    summaryItem: { alignItems: 'center' },
    summaryNumber: { fontFamily: 'Syne_700Bold', fontSize: 28, fontWeight: '700' },
    summaryLabel: { fontFamily: 'Syne_700Bold', fontSize: 11, color: DT.muted, marginTop: 4 },
    ...
    statsTitle: { fontFamily: 'Syne_700Bold', fontSize: 13, fontWeight: '700', color: DT.white, marginBottom: 14, letterSpacing: 0.5 },
    statsGrid: { flexDirection: 'row', flexWrap: 'wrap' },
    statBox: { width: '33%', alignItems: 'center', marginBottom: 12 },
    statValue: { fontFamily: 'Syne_700Bold', fontSize: 22, fontWeight: '700' },
    statLabel: { fontFamily: 'Syne_700Bold', fontSize: 10, color: DT.muted, marginTop: 4, textAlign: 'center' },
    ...
    reportBtnTitle: { fontFamily: 'Syne_700Bold', fontSize: 14, fontWeight: '600', color: DT.white },
    reportBtnSub: { fontFamily: 'Syne_700Bold', fontSize: 11, color: DT.muted, marginTop: 2 },
  ...
  if (loading) {
    return (
      <View style={s.container}>
        ...
        <View style={s.loadingWrap}><ActivityIndicator size="large" color={DT.blue} /><Text style={{ color: DT.muted, marginTop: 10 }}>Loading...</Text></View>
      </View>
    );
  }
  ...
  return (
    <View style={s.container}>
      ...
      {/* Tabs */}
      <View style={s.tabs}>
        <TouchableOpacity style={[s.tab, activeTab === 'attendance' && { borderBottomWidth: 2, borderBottomColor: DT.cyan }]} onPress={() => setActiveTab('attendance')}>
          <Text style={[s.tabText, { color: activeTab === 'attendance' ? DT.cyan : DT.muted }]}>Daily Attendance</Text>
        </TouchableOpacity>
        <TouchableOpacity style={[s.tab, activeTab === 'reports' && { borderBottomWidth: 2, borderBottomColor: DT.cyan }]} onPress={() => setActiveTab('reports')}>
          <Text style={[s.tabText, { color: activeTab === 'reports' ? DT.cyan : DT.muted }]}>Reports</Text>
        </TouchableOpacity>
      </View>
      ...
      {activeTab === 'attendance' ? (
        ...
        >
          ...
          <View style={s.filterRow}>
            ...
            <TouchableOpacity
              style={[s.filterBtn, { backgroundColor: selectedChild ? DT.green2 : DT.panel }]}
            ...
            return (
              <View key={child.id} style={s.childCard}>
                ...
                <View style={s.childInfo}>
                  <View style={[s.childAvatar, { backgroundColor: `${DT.blue}15`, borderColor: `${DT.blue}35` }]}>
                    <Ionicons name="person" size={18} color={DT.blue} />
                  </View>
                ...
                {record ? (
                  <View style={[s.statusBadge, { backgroundColor: getStatusColor(record.status) }]}>
                    <Ionicons name={getStatusIcon(record.status) as keyof typeof Ionicons.glyphMap} size={14} color={DT.white} />
                    <Text style={s.statusText}>{record.status}</Text>
                  </View>
                ) : (
                  <View style={s.actionBtns}>
                    <TouchableOpacity style={[s.actionBtn, { backgroundColor: DT.green2 }]} onPress={() => markAttendance(child.id, 'present')}>
                      <Ionicons name="checkmark" size={16} color={DT.bg} />
                    </TouchableOpacity>
                    <TouchableOpacity style={[s.actionBtn, { backgroundColor: DT.red }]} onPress={() => markAttendance(child.id, 'absent')}>
                      <Ionicons name="close" size={16} color={DT.white} />
                    </TouchableOpacity>
                    <TouchableOpacity style={[s.actionBtn, { backgroundColor: DT.amber }]} onPress={() => markAttendance(child.id, 'excused')}>
                      <Ionicons name="time" size={16} color={DT.bg} />
                    </TouchableOpacity>
                  </View>
                )}
              </View>
          ...
          <View style={s.summaryCard}>
            ...
            <View style={s.summaryRow}>
              <View style={s.summaryItem}>
                <Text style={[s.summaryNumber, { color: DT.green2 }]}>{attendance.filter(a => a.status === 'present').length}</Text>
                <Text style={s.summaryLabel}>Present</Text>
              </View>
              <View style={s.summaryItem}>
                <Text style={[s.summaryNumber, { color: DT.red }]}>{attendance.filter(a => a.status === 'absent').length}</Text>
                <Text style={s.summaryLabel}>Absent</Text>
              </View>
              <View style={s.summaryItem}>
                <Text style={[s.summaryNumber, { color: DT.amber }]}>{attendance.filter(a => a.status === 'excused').length}</Text>
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          ...
          <View style={s.reportTypeRow}>
            {(['daily', 'weekly', 'monthly'] as const).map((type) => (
              <TouchableOpacity
                key={type}
                style={[s.reportTypeBtn, reportType === type && { backgroundColor: DT.blue }]}
                onPress={() => setReportType(type)}
              >
                <Text style={[s.reportTypeText, { color: reportType === type ? DT.white : DT.muted }]}>
          ...
          <View style={s.statsCard}>
            ...
            <View style={s.statsGrid}>
              <View style={s.statBox}>
                <Text style={[s.statValue, { color: DT.blue }]}>{children.length}</Text>
                <Text style={s.statLabel}>Total Children</Text>
              </View>
              <View style={s.statBox}>
                <Text style={[s.statValue, { color: DT.cyan }]}>{attendance.length}</Text>
                <Text style={s.statLabel}>Records Today</Text>
              </View>
              <View style={s.statBox}>
                <Text style={[s.statValue, { color: DT.green2 }]}>
          ...
          {[
            { icon: 'document-text', title: 'Attendance Report', sub: 'Export as CSV', color: DT.blue },
            { icon: 'bus', title: 'Trip Summary', sub: 'All trips for period', color: DT.cyan },
            { icon: 'people', title: 'Driver Performance', sub: 'Compliance & ratings', color: DT.amber },
          ].map((btn, i) => (
